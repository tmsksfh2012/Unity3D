...
public class GameManager : MonoBehaviour
{
            ...
            if(Pos_Idx < 20) {
                ...
                if(pos == 0) {
                    if(idx % 20 == 0) {
                        ...
                        Platform_List[idx % 20].transform.position = Platform_List[20 - 1].transform.position
                        + new Vector3(-1f, 0.5f, 0);
                    }
                    else {
                        Platform_Check_List[idx % 20 - 1] = pos;
                        Platform_List[idx % 20].transform.position = Platform_List[idx % 20 - 1].transform.position
                        + new Vector3(-1f, 0.5f, 0);
                    }
                }
                else {
    ...
    void Check_Platform(int idx, int direction) {
        if(Platform_Check_List[idx % 20] == direction) {
            Character_Pos_Idx++;
            Character.transform.position = Platform_List[Character_Pos_Idx % 20].transform.position +
